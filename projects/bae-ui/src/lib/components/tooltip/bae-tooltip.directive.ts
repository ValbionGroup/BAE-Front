 ...
 *
 *   <button baeTooltip="Lancer l'algo">…</button>
 *   <button baeTooltip="Notifications" baeTooltipPlacement="bottom-end">…</button>
 */
@Directive({
  selector: '[baeTooltip]',
  host: {
...
export class BfdTooltip implements OnDestroy {
  readonly title = input.required<string>({ alias: 'baeTooltip' });
  readonly description = input<string>('', { alias: 'baeTooltipDescription' });
  readonly placement = input<Placement>('top', { alias: 'baeTooltipPlacement' });
  readonly delay = input<number>(250, { alias: 'baeTooltipDelay' });
