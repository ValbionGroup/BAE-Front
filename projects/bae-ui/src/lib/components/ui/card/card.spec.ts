...
describe('Card', () => {
  ...
  it('renders with default padding and the bg-surface class', () => {
    ...
    const cards = host.querySelectorAll<HTMLElement>('bae-card > div');
  ...
  it('lets the content fill the card and own its scrolling', () => {
    ...
    // Les classes de flex posées sur `<bae-card>` s'arrêtent à l'hôte : sans
  ...
  it('applies a custom padding value', () => {
    const host = fixture.nativeElement as HTMLElement;
    const customCard = host.querySelectorAll<HTMLElement>('bae-card > div')[1];
