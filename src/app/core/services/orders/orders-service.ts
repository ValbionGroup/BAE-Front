...
function buildSeedOrders(eventId: string): Order[] {
  const now = Date.now();
  const items = (list: Array<{ recipeId: string; recipeName: string; quantity: number; notes?: string }>): OrderItem[] =>
    list;

  return [
    {
      id: 'o1', number: 1, eventId,
      items: items([{ recipeId: 'r1', recipeName: 'Mojito', quantity: 2 }]),
      status: 'pending', createdAt: now - 120_000, updatedAt: now - 120_000,
    },
    {
      id: 'o2', number: 2, eventId,
      items: items([
        { recipeId: 'r6', recipeName: 'Merguez frites', quantity: 1 },
        { recipeId: 'r2', recipeName: 'Panaché', quantity: 1 },
      ]),
      status: 'in_progress', createdAt: now - 240_000, updatedAt: now - 60_000,
    },
    {
      id: 'o3', number: 3, eventId,
      items: items([{ recipeId: 'r4', recipeName: 'Plateau apéro', quantity: 1, notes: 'Sans gluten si possible' }]),
      status: 'ready', createdAt: now - 360_000, updatedAt: now - 30_000,
    },
    {
      id: 'o4', number: 4, eventId,
      items: items([{ recipeId: 'r3', recipeName: 'Sangria', quantity: 3 }]),
      status: 'pending', createdAt: now - 90_000, updatedAt: now - 90_000,
    },
    {
      id: 'o5', number: 5, eventId,
      items: items([
        { recipeId: 'r5', recipeName: 'Vodka Orange', quantity: 2 },
        { recipeId: 'r1', recipeName: 'Mojito', quantity: 1 },
      ]),
      status: 'completed', createdAt: now - 600_000, updatedAt: now - 300_000,
    },
    {
      id: 'o6', number: 6, eventId,
      items: items([{ recipeId: 'r6', recipeName: 'Merguez frites', quantity: 2 }]),
      status: 'pending', createdAt: now - 45_000, updatedAt: now - 45_000,
    },
  ];
}
  ...
  private applyWsMessage(msg: WsMessage): void {
    switch (msg.type) {
      case 'order.created':
        ...
        this._allOrders.update(orders =>
          orders.some(o => o.id === msg.payload.id)
            ? orders
            : [...orders, msg.payload]
        );
        break;
