 ...
 *   @if (open()) {
 *     <div [bfdFloating]="trigger" bfdPlacement="bottom-end">...</div>
 *   }
 */
@Directive({
  selector: '[bfdFloating]',
})
export class FloatingDirective {
  readonly anchor = input.required<HTMLElement | null>({ alias: 'bfdFloating' });
  readonly placement = input<Placement>('bottom-end', { alias: 'bfdPlacement' });
  readonly distance = input<number>(6, { alias: 'bfdDistance' });
