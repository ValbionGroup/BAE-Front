...
| 2    | **46 + 47** | Le plafond de précommandes n'est appliqué **nulle part**, et rien n'empêche un compte de le consommer entier. Les deux vont ensemble. |
...
| 46     | Contrainte d'unicité `(user_id, event_id)` sur `pre_orders` — sans elle un même compte consomme tout le plafond                                                                                                                                                                                                                                                                                                               | Oui. **Confirmée ouverte le 2026-08-26** : `\d pre_orders` ne porte que les deux clés étrangères, aucun index unique. `H2 §38.10`                                                                                                                                     |
| 47     | ⚠️ **Le plafond de précommandes n'est appliqué nulle part** (constaté le 2026-08-26, la tâche disait « vérifier que ça a été écrit » — ça ne l'a pas été). `pre_order_quote_service` ne contrôle que l'heure de fermeture ; `fulfilPreOrder` (`payment_service.ts:135`) insère dans `pre_orders` **sans contrôle ni verrou**. `events.capacity` ne sert qu'à l'affichage (`remaining`, `open` dans `public_catalog_service`). | Oui, et **à faire avec la 46** : deux paiements concurrents passent tous les deux. `H2 §38.10, H1 §10.4`                                                                                                                                                              |
...
| 101 | Demander les **post-logout redirect URIs de production** à EirbWare — c'est EirbConnect, donc toujours eux. ⚠️ Les URI à demander ne sont plus `dashboard.bae.eirb.fr` / `order.bae.eirb.fr` : **l'hébergement passe chez Dyjix** (2026-08-26), les domaines de production sont à fixer d'abord.           | En dev, `scripts/setup-dev-keycloak.sh` suffit. `H2 §39.6`                                                                                                                                                                                                       |
