...
Bouton **Remise** en caisse, écriture dans `order_discounts` avec `applied_by_user_id`, permission
`order:discount`. Arbitrages tranchés le 2026-08-27 : **montant en euros**, sur la **commande
entière**, motif pris dans une **liste ouverte** (quatre motifs courants + « Autre » libre).

#### Le choix du montant a supprimé deux arbitrages

Saisir des euros plutôt qu'un pourcentage **clôt les tâches 92 et 93** : plus de pourcentages à
cumuler (multiplicatif ou additif), plus de centimes à arrondir. Elles ne sont pas « à faire plus
tard », elles n'ont plus d'objet pour ce lot.

#### La moitié de la tâche 85 était déjà tranchée — par le schéma

Sa question la plus lourde — « produit offert à un partenaire : −100 % ou notion _offert_ distincte
(vente à 0 € vs charge, **pas le même chiffre**) ? » — a sa réponse dans `order_service` :

> « Un écart entre prix public et prix facturé est une **créance** quand la commande porte une
> catégorie de prise en charge, une **remise consentie** sinon. »

D'où l'invariant `gross = total + discount + sponsored`. « Offert à un partenaire » est une
`sponsorship_category`, pas une remise. L'écran les affiche donc sur **deux lignes distinctes** :
les confondre à l'écran, c'est les confondre au bilan. Ce qui reste ouvert de la 85 (staff, happy
hour, geste commercial systématique) ne bloque plus la remise libre.

#### Trois points où l'argent pouvait déraper

| Point               | Ce qui a été fait                                                                                                                                                                                                     |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Total négatif       | La remise est **ramenée au dû** (`Math.min`), pas refusée : refuser bloquerait un « c'est cadeau » légitime. Le front applique le même plafond et le **dit**, plutôt que de laisser le serveur corriger en silence.   |
| Encaissement faussé | `writeOrder` crée la transaction comptable à `draft.totalCents`. La remise entre donc dans le **draft**, sans quoi la commande afficherait 4 € et la caisse encaisserait 5.                                           |
| Terminal carte      | `openCardPayment` sérialise le draft entier dans `payment.intent` et facture `draft.totalCents` : la remise voyage avec l'**ouverture** du paiement, pas avec sa confirmation. Un test dédié le garde des deux côtés. |

⚠️ **La garde ne peut pas vivre sur la route.** `POST /events/:id/orders` sert aussi tous les
encaissements sans remise, qui doivent rester ouverts à `order:write` seul. Le contrôle est donc
conditionnel (`assertMayDiscount`), **avant tout écrit**, dans les deux contrôleurs — un refus ne
doit laisser derrière lui ni commande, ni transaction, ni terminal allumé.

⚠️ **`created_at` posé à la main.** Un `insert` brut ne passe pas par Lucid, donc `autoCreate` ne
s'applique pas, et les deux colonnes d'horodatage sont NOT NULL sans défaut. Trouvé par un test, pas
en production.

#### Ce qui reste à confirmer par le bureau

`order:discount` est donné au **Trésorier** seul parmi les rôles non pleins (President et
Administrateur ont tout). Un Coordinateur qui tient le comptoir peut donc encaisser **sans** pouvoir
accorder de remise. C'est un réglage d'écran des rôles, pas une règle du code — à élargir si le
bureau le veut.

Back : **760 tests, 0 échec**, `tsc --noEmit` vert. Front : **868** (dashboard, dont 14 nouveaux),
109 (`bae-ui`), 115 (`bae-public`), `ng build` vert. Le chemin carte a été cassé volontairement pour
vérifier que son test rougissait — il l'a fait, seul.
...
| ~~53~~ | ~~Bouton **Remise** en caisse~~                                                                                                               | ✅ **Fait le 2026-08-27.** Montant en euros, commande entière, motif en liste ouverte, `order:discount`. Voir « Livré ».                                                                                                                                                                                                                                                          |
...
| #      | Arbitrage                                                                                                                                            | Note                                                                                                                                            |
| ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~92~~ | ~~Cumul des remises : multiplicatif ou additif~~                                                                                                     | ✅ **Sans objet depuis le 2026-08-27** : la remise se saisit en euros, il n'y a plus de pourcentages à cumuler.                                 |
| ~~93~~ | ~~Arrondi au centime ou aux 10 centimes~~                                                                                                            | ✅ **Sans objet depuis le 2026-08-27** : un montant en centimes entiers ne s'arrondit pas.                                                      |
